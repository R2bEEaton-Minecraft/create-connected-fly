        ...
        for (BlockModelPart part : getMaterialParts(world, pos, material, random, getModelOf(material))) {
            ...
            for (boolean row : Iterate.trueAndFalse) {
                for (boolean column : Iterate.trueAndFalse) {
                    ...
                    boolean finalRow = row;
                    boolean finalColumn = column;
                    AABB finalBb1 = bb1;
                    processQuads(part.getQuads(null), finalRow, finalColumn, finalBb1, offset, builder::addUnculledFace);
                    for (Direction direction : Iterate.directions) {
                        Direction d = direction;
                        processQuads(part.getQuads(direction), finalRow, finalColumn, finalBb1, offset, quad -> builder.addCulledFace(d, quad));
                    }
                }
            }
